...
def _handle_transition(printer, prev, cur, status, webhook, alerts):
    ...
    if cur == "complete" and prev in ("printing", "paused") and alerts.get("complete", True):
        discord_send(webhook, f"{name} - Print complete", f"File: **{fname}**", 0x34E0A1)
    elif "error" in str(cur) and alerts.get("error", True):
        discord_send(webhook, f"{name} - Error", f"State: {cur}\nFile: {fname}", 0xFF6B6B)
    elif cur == "offline" and prev != "offline" and alerts.get("offline", False):
        discord_send(webhook, f"{name} - Offline", "The printer is not responding.", 0xFF9F43)
    elif prev == "offline" and cur != "offline" and alerts.get("offline", False):
        discord_send(webhook, f"{name} - Online", "The printer is responding again.", 0x6C8CFF)
...
# --------------------------------------------------------------------------
# Printer status connectors
# --------------------------------------------------------------------------
from connectors import empty_status, fetch_status
...
def api_status_q():
    ...
    status.setdefault("state", "offline")
    status.setdefault("progress", 0.0)
    status.setdefault("temps", {})
    status.setdefault("sensors", {})
    status.setdefault("system", {})
    status["ok"] = bool(status.get("online")) and not bool(status.get("error"))
    status["type"] = p.get("type")
    status["capabilities"] = protocol_capabilities(p.get("type"))
...
def api_stats_csv_q():
    ...
    w.writerow(["date", "file", "status", "duration_min", "filament_mm", "weight_g", "type"])
...
def api_monitor_test():
    ...
    ok = discord_send(wh, "PrintWatch - Test", "Discord alerts work.", 0x6C8CFF)
    return (jsonify({"ok": True}) if ok else (jsonify({"error": "Send failed"}), 502))
